...
def ConvergenceTest(ODEsolve, Tmax, n, f, ic, method, order): 
    ...
    diff1 = (Y1 - Y2[::2])                 # [::2] skips every other indices in the array and 4 skips every fourth
...
plt.grid() 
plt.subplot(2,1,2)
plt.plot(T[::4],d1/d2, label=r'$\frac{Y - Y/2}{2\left(Y/2 - Y/4\right)}$') # plotting Ydt - Ydt/2 / 2*Ydt/2-Ydt/4 to show that euler is first order convergent
plt.xlabel(r'$Time$ $(s)$')
